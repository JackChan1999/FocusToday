...
public class PhotoSelectionFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener,
    ...
    private void initData() {
        int count = BaseApplication.getSpUtils().getInt("index_selection");
        lastupdatetime = BaseApplication.getSpUtils().getLong("lastupdatetime_selection");
        if (count != -1) {
            if ((lastupdatetime + (24 * 60 * 60 * 1000)) < System.currentTimeMillis()) {
                lastupdatetime = System.currentTimeMillis();
                BaseApplication.getSpUtils().putLong("lastupdatetime_selection", lastupdatetime);
            ...
            indexId = 90000;
            BaseApplication.getSpUtils().putLong("lastupdatetime_selection", System.currentTimeMillis());
            BaseApplication.getSpUtils().putInt("index_selection", indexId);
        }

        index = indexId;
        requestData(RequestUrl.getPicHotUrl(indexId), false);
    }
    ...
    public void onRefresh() {
        ...
        isRefresh = true;
        index = index + 5;
        requestData(RequestUrl.getPicHotUrl(index),false);
        BaseApplication.getSpUtils().putInt("index_selection",index);
    }
    ...
    public void onLoadMoreRequested() {
        currentPagte++;
        indexId = indexId - 10;
        requestData(RequestUrl.getPicHotUrl(indexId), true);
    }
    ...
    private class PhotoAdapter extends CommonAdapter<PhotoBean> {
        ...
        protected void convert(ViewHolder holder, PhotoBean data, int position) {
            ...
            Glide.with(getActivity()).load(data.cover).placeholder(new ColorDrawable(Color
                    .parseColor("#eaeaea"))).into(iv);
        }
    }
