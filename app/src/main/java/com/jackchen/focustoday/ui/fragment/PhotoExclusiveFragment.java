...
public class PhotoExclusiveFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener,
    ...
    private void initData() {
        ...
        int count = BaseApplication.getSpUtils().getInt("index_exclusive");
        lastupdatetime = BaseApplication.getSpUtils().getLong("lastupdatetime_exclusive");
        if (count != -1) {
            if ((lastupdatetime + (24 * 60 * 60 * 1000)) < System.currentTimeMillis()) {
                lastupdatetime = System.currentTimeMillis();
                BaseApplication.getSpUtils().putLong("lastupdatetime_exclusive", lastupdatetime);
            ...
            indexId = 90000;
            BaseApplication.getSpUtils().putLong("lastupdatetime_exclusive", System.currentTimeMillis());
            BaseApplication.getSpUtils().putInt("index_exclusive", indexId);
        }

        index = indexId;
        requestData(RequestUrl.getPicExclusiveUrl(indexId), false);
    }
    ...
    public void onRefresh() {
        ...
        isRefresh = true;
        index = index + 5;
        requestData(RequestUrl.getPicExclusiveUrl(index),false);
        BaseApplication.getSpUtils().putInt("index_exclusive",index);
    }
    ...
    public void onLoadMoreRequested() {
        currentPagte++;
        indexId = indexId - 10;
        requestData(RequestUrl.getPicExclusiveUrl(indexId), true);
    }
