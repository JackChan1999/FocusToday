...
import com.jackchen.focustoday.base.BaseApplication;
import com.jackchen.focustoday.bean.PhotoBean;
import com.jackchen.focustoday.bean.Video;
import com.jackchen.focustoday.glidepalette.GlidePalette;
...
public class PhotoSportFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener,
    ...
    public View initView(LayoutInflater inflater, ViewGroup container) {
        View rootView = inflater.inflate(R.layout.layout_photo_sport, container, false);
    ...
    private void initData() {
        ...
        int count = BaseApplication.getSpUtils().getInt("index_sports");
        lastupdatetime = BaseApplication.getSpUtils().getLong("lastupdatetime_sports");
        if (count != -1) {
            if ((lastupdatetime + (24 * 60 * 60 * 1000)) < System.currentTimeMillis()) {
                lastupdatetime = System.currentTimeMillis();
                BaseApplication.getSpUtils().putLong("lastupdatetime_sports", lastupdatetime);
            ...
            indexId = 90000;
            BaseApplication.getSpUtils().putLong("lastupdatetime_sports", System.currentTimeMillis());
            BaseApplication.getSpUtils().putInt("index_sports", indexId);
        }

        index = indexId;
        requestData(RequestUrl.getPicSportsUrl(indexId), false);
    }
    ...
    private void parseData(String result, boolean isLoadMore) {
        ...
        if (!isLoadMore) {
            if (isRefresh) {
                ...
                mdata = data;
                mAdapter = new PhotoAdapter(getActivity(), R.layout.list_item_sport, mdata);
    ...
    public void onRefresh() {
        ...
        isRefresh = true;
        index = index + 5;
        requestData(RequestUrl.getPicSportsUrl(index), false);
        BaseApplication.getSpUtils().putInt("index_sports",index);
    }
    ...
    public void onLoadMoreRequested() {
        currentPagte++;
        indexId = indexId - 10;
        requestData(RequestUrl.getPicSportsUrl(indexId), true);
    }
    ...
    private class PhotoAdapter extends CommonAdapter<PhotoBean> {
        ...
        protected void convert(ViewHolder holder, PhotoBean data, int position) {
            ...
            tvTitle.setText(data.setname);
            Glide.with(getActivity())
                    .load(data.cover)
                    .placeholder(new ColorDrawable(Color.parseColor("#eaeaea")))
                    .listener(GlidePalette.with(data.cover).use(GlidePalette.Profile.MUTED).intoBackground(tvTitle))
                    .into(iv);
        }
    }
