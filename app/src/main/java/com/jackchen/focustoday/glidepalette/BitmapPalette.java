package com.jackchen.focustoday.glidepalette;
...
public abstract class BitmapPalette {
    ...
    private void assertTargetsIsNotEmpty() {
        if (this.targets.isEmpty()) {
            throw new UnsupportedOperationException("You must specify StorageVolume palette with use(Profile.Profile)");
        }
    }
    ...
    protected void apply(Palette palette, boolean cacheHit) {
        ...
        for (PaletteTarget target : targets) {
            ...
            for (Pair<View, Integer> t : target.targetsBackground) {
                ...
                //Only crossfade if we're not coming from StorageVolume cache hit.
